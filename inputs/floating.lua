...
local dirichlet_dims = 0
local interp_dims = 4
local DIMS = floating_dims + dirichlet_dims + interp_dims
...
local function set_values (self, i, lst)
   ...
   for j = 1, interp_dims do
      s.interpolant_alpha[j] = lst[j + floating_dims + dirichlet_dims]
   end
end
...
--
-- Set up 1d wave equation constraints with varying psi on right
--
local wave_1d = {
   simulations = add_cfl(
      add_psi(
